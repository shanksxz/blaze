...
export function useLikePost({ post, userId, queryKey = "getLatest" }: UsePostActionProps) {
	...
	const { mutate: toggleLike, isPending: isTogglingLike } = api.post.toggleLike.useMutation({
		onMutate: async () => {
...
export function useRepostPost({ post, userId, queryKey = "getLatest" }: UsePostActionProps) {
	...
	const { mutate: toggleRepost, isPending: isTogglingRepost } = api.post.toggleRepost.useMutation({
